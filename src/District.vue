<template>
    <div>
        <multiselect v-model="selected" :options="optiontxt" :close-on-select="true" :clear-on-select="true"
            :hide-selected="false" :preserve-search="true" :option-height="30" :block-keys="['Delete']"
            disabledReason="true" placeholder="กรุณาเลือก" select-label="เลือก" deselect-label="ลบ"
            selectedLabel="เลือกอยู่" label="district" @select="onSelect"
            v-validate="iRequire" 
            :data-vv-name="iKey"
            ...
            <template slot="singleLabel" slot-scope="props">
                <div class="option__desc">
                    <span class="option__title">{{ props.option.district }}</span>
                </div>
            </template>
...
<script>
    ...
    import district from './ThailandAddress/OnlyDistrict.json'
    ...
    export default {
        ...
        computed: {
            ...
            selected: {
                get() {
                    const index = this.optiontxt.findIndex(data => data.district == this.Obj[this.iKey])

                    if (index >= 0) {
                        const data = [{
                            district: this.optiontxt[index].district,
